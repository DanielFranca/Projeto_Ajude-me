...
export default function LoginForPsicologo({
  handleBottonClickMain,
  handleBottonClickBeneficiario,
}) {
  const navigate = useNavigate();
  var userPsicologo = json;
  async function handleLoginSubmitPsicologo(event) {
    event.preventDefault();
    const formData = new FormData(event.target);
    const data = Object.fromEntries(formData);
    // console.log("DADOS DO LOGIN DO PSICOLOGO");
    // console.log(data);
    try {
      await axios
        .post("http://localhost:5000/api/v1/auth/loginPsicologo", {
          email: data.email,
          password: data.password,
        })
        .then((response) => {
          userPsicologo = response.data;
          // console.log("User Psicologo: ", userPsicologo);
          localStorage.setItem("userPsicologo", userPsicologo);
          localStorage.setItem("accessToken", userPsicologo.accessToken);
          localStorage.setItem("refreshToken", userPsicologo.refreshToken);
          localStorage.setItem(
            "idPsicologo",
            userPsicologo.psicologo.idPsicologo
          );
        });
  ...
  return (
    <form onSubmit={handleLoginSubmitPsicologo}>
